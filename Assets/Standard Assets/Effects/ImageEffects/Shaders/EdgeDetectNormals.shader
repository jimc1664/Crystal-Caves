...
Shader "Hidden/EdgeDetect" { 
	...
	sampler2D _MainTex;
	uniform float4 _MainTex_TexelSize, _Color;
	...
	{	
		...
		Sobel = 1.0-pow(saturate(Sobel-0.1), _Exponent);
	//	return Sobel * lerp(tex2D(_MainTex, i.uv[0].xy), _BgColor, _BgFade);
		//return lerp( _Color*(1-Sobel),  lerp(tex2D(_MainTex, i.uv[0].xy), _BgColor, _BgFade), Sobel  );
		float4 base = tex2D(_MainTex, i.uv[0].xy);//, satBase = saturate( pow(base,4));
			
			float satBase = (base.r + base.g + base.b)*10;
	satBase = saturate(satBase*satBase);

		float4 col = lerp( base, _BgColor, _BgFade);
		float s2 = max(1.1-Sobel*1.2, -1);
			return Sobel *col + s2*_Color*satBase;// + s2*_Color * col;
	}
